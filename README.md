...
## Push-напоминание по субботам

Сервер раз в 15 минут проверяет активные устройства. Если в часовом поясе хотя бы
одного устройства пользователя суббота и попадает заданное время напоминания (по
умолчанию `10:00`), пользователь имеет хотя бы одного новообращённого и уведомление
ещё не отправлялось в эту локальную субботу, оно уйдёт на все его активные устройства.

Отдельно через 24 часа после добавления каждого новообращённого сервер отправляет
его владельцу напоминание написать этому человеку. Оно отправляется один раз на все
активные устройства владельца.
...
{"believers_friday_reminder_enabled":true,"believers_friday_reminder_time":"10:00"}
```
